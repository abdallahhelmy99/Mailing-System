...
class dbHelper {
  ...
  _onCR(Database db, int v) async {
    ...
    await db.execute('CREATE TABLE "Mail" (emailID INTEGER PRIMARY KEY AUTOINCREMENT, subject TEXT NOT NULL, body TEXT NOT NULL, trash Boolean, important Boolean , spam Boolean , isRead Boolean, draft Boolean, date DATETIME, senderID INTEGER NOT NULL, receiverID INTEGER NOT NULL, isSent Boolean)');
  ...
  Future<List<Mail>> readInbox() async {
    Database? mydb = await db;
    List<Map<String, dynamic>> response =
        await mydb!.rawQuery("SELECT * FROM Mail WHERE receiverID = ${globalVariables.currentUser!.userID} AND trash = 'false' AND spam = 'false' AND draft = 'false' AND important = 'false'");
    return List.generate(response.length, (i) {
        return Mail(
  ...
  Future<List<Mail>> readDraft() async {
    Database? mydb = await db;
    List<Map<String, dynamic>> response =
        await mydb!.rawQuery("SELECT * FROM Mail WHERE receiverID = ${globalVariables.currentUser!.userID} AND draft = 'true'");
    return List.generate(response.length, (i) {
        return Mail(
        emailID: response[i]['emailID'],
        subject: response[i]['subject'],
  ...
  Future<List<Mail>> readSpam() async {
    Database? mydb = await db;
    List<Map<String, dynamic>> response =
        await mydb!.rawQuery("SELECT * FROM Mail WHERE receiverID = ${globalVariables.currentUser!.userID} AND spam = 'true'");
    return List.generate(response.length, (i) {
        return Mail(
         emailID: response[i]['emailID'],
        subject: response[i]['subject'],
  ...
  Future<List<Mail>> readImportant() async {
    Database? mydb = await db;
    List<Map<String, dynamic>> response =
        await mydb!.rawQuery("SELECT * FROM Mail WHERE receiverID = ${globalVariables.currentUser!.userID} AND important = 'true'");
    return List.generate(response.length, (i) {
        return Mail(
       emailID: response[i]['emailID'],
        subject: response[i]['subject'],
  ...
  Future<List<Mail>> readTrash() async {
    Database? mydb = await db;
    List<Map<String, dynamic>> response =
        await mydb!.rawQuery("SELECT * FROM Mail WHERE receiverID = ${globalVariables.currentUser!.userID} AND trash = 'true'");
    return List.generate(response.length, (i) {
        return Mail(
        emailID: response[i]['emailID'],
        subject: response[i]['subject'],
   ...
   Future<List<Mail>> readSent() async {
    Database? mydb = await db;
    List<Map<String, dynamic>> response =
        await mydb!.rawQuery("SELECT * FROM Mail WHERE senderID = ${globalVariables.currentUser!.userID} AND isSent = true");
    return List.generate(response.length, (i) {
        return Mail(
        emailID: response[i]['emailID'],
        subject: response[i]['subject'],
