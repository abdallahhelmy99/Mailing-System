...
class dbHelper {
  ...
  _onCR(Database db, int v) async {
    ...
    await db.execute(
        'CREATE TABLE "Mail" (emailID INTEGER PRIMARY KEY AUTOINCREMENT, subject TEXT NOT NULL, body TEXT NOT NULL, trash Boolean, important Boolean , spam Boolean , isRead Boolean, date DATETIME, senderID INTEGER NOT NULL, receiverID INTEGER NOT NULL, isSent Boolean)');
    await db.execute(
        'CREATE TABLE "Contacts" (userID integer,contact_ID integer,contactname TEXT NOT NULL , contactemail TEXT NOT NULL,  phonenum TEXT NOT NULL, relation TEXT NOT NULL)');
  ...
  Future<List<Mail>> readMyEmails() async {
    ...
    List<Map<String, dynamic>> response = await mydb!.rawQuery(
        "SELECT * FROM Mail WHERE receiverID = ${globalVariables.currentUser!.userID}");
    return List.generate(response.length, (i) {
      return Mail(
